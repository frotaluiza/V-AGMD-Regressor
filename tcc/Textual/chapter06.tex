...
A arquitetura selecionada foi a \texttt{KerasMLP\_ZohanResidual\_Restricted}, por apresentar o melhor equilíbrio entre erro em validação cruzada e complexidade estrutural segundo a regra de seleção definida no Capítulo de Metodologia. O modelo obteve RMSE$_{Flux}$ em validação cruzada de aproximadamente 0,061 e RMSE OOF de 0,061, demonstrando alta estabilidade entre as duas estimativas. Na comparação entre famílias, a complexidade foi medida pelo número total de parâmetros treináveis de cada arquitetura, permitindo posicionar modelos lineares (18 parâmetros) e redes neurais em uma escala comum. Como as buscas foram conduzidas por target, as contagens diferem: as baselines de \(T_{\mathrm{alim,out}}\) e \(T_{\mathrm{ref,out}}\) (arquitetura \([128,64]\)) totalizam 9219 parâmetros, enquanto a baseline de \(Flux\) (arquitetura \([128,64,32]\)) totaliza 11203. As arquiteturas híbridas HPD e HRNN acrescentam 384 parâmetros relativos à primeira camada com oito entradas, totalizando 9603 ou 11587, respectivamente. O ZohanResidual preserva a contagem da baseline neural correspondente a cada alvo (9219 ou 11203), diferindo apenas na forma de integração com o modelo físico.
...
A arquitetura e os hiperparâmetros selecionados nessa etapa passaram a compor a configuração da rede neural de referência, utilizada como base para as estratégias híbridas avaliadas posteriormente no \textit{pipeline}. Para o alvo $Alim\_T\_out$, a configuração selecionada foi composta por uma camada oculta com 256 neurônios, ativação \textit{logistic} e regularização $\alpha=0,1$, conforme determinado pela busca exaustiva realizada na etapa anterior. Essa configuração foi adotada como baseline para as arquiteturas híbridas nos três alvos, mantendo-se a mesma estrutura de rede para garantir comparabilidade. A partir dessa configuração inicial, a etapa de ajuste da arquitetura híbrida ZohanResidual manteve a estrutura da rede fixa, variando apenas o hiperparâmetro de regularização $L_2$. Essa escolha permitiu avaliar se, após a incorporação da informação proveniente do modelo físico reduzido, o desempenho do modelo continuava sensível à intensidade da regularização.
...
ZohanResidual (base Alim) & \textbf{0.141} & 0.317 & 0.060 \\
ZohanResidual (base Ref) & 0.240 & \textbf{0.264} & 0.056 \\
ZohanHPD (base Flux) & 0.155 & 0.295 & \textbf{0.060} \\
\bottomrule
\end{tabular}
\end{table}
...
Os resultados indicam que os modelos híbridos com otimização por target apresentaram redução expressiva do erro em relação ao modelo físico reduzido para as três variáveis analisadas. Para o fluxo de permeado, o modelo ZohanHPD (otimizado para o Flux) obteve RMSE de 0,060, superando o melhor resultado de 0,061 reportado no TCC anterior. Para a temperatura de saída da alimentação, o modelo ZohanResidual (otimizado para Alim) alcançou RMSE de 0,141, representando uma redução de 51\% em relação ao resultado anterior do TCC (0,290). Para a temperatura de saída do circuito de resfriamento, o modelo ZohanResidual (otimizado para Ref) obteve RMSE de 0,264, representando uma melhoria de 31\% em relação ao valor anterior (0,385).

Esse resultado mostra que a estratégia de otimização por target — isto é, realizar buscas independentes de hiperparâmetros para cada variável de saída — permitiu que cada arquitetura híbrida fosse ajustada especificamente para o alvo de interesse, em vez de um único conjunto de parâmetros otimizado apenas para o fluxo de permeado. Como consequência, foi possível obter ganhos simultâneos nas três variáveis de saída, superando a limitação observada na abordagem anterior, em que as temperaturas apresentavam desempenho inferior ao de modelos lineares simples.

A principal contribuição desta análise está em demonstrar que, para o sistema V-AGMD analisado, a abordagem de otimização por target combinada com arquiteturas híbridas proporcionou o melhor desempenho preditivo já obtido para as três variáveis de saída simultaneamente. O modelo ZohanResidual com baseline otimizado para Alim obteve RMSE de 0,141 para $Alim\_T\_out$, enquanto o ZohanResidual com baseline Ref alcançou 0,264 para $Ref\_T\_out$, e o ZohanHPD com baseline Flux atingiu 0,060 para o fluxo de permeado. Esse resultado é particularmente relevante porque demonstra que a otimização individualizada por saída, aliada à incorporação de informação física via arquiteturas híbridas, pode superar as limitações de modelos multitarefa com otimização única.