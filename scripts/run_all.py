#!/usr/bin/env python3
"""
Pipeline completo: Stage 0 + Stage 1 + Stage 2 + consolidação final.
Equivalente a: t2.py + t3.py + consolidar_resultados.py

Uso:
  python scripts/run_all.py --csv <caminho_do_csv>
"""
...
def parse_cli_args():
    parser = argparse.ArgumentParser(description="AGMD unified pipeline: classical + KerasMLP + restricted hybrids + consolidation.")
...
def main():
    ...
    # ============================================================
    # STAGE 0 - Classical models (grid search, rápido)
    # ============================================================
    classical_families = [
    ...
    # ============================================================
    # STAGE 1 - Baseline KerasMLP (random search)
    # ============================================================
    import tensorflow as tf
    ...
    logger.info("=" * 72)
    logger.info("STAGE 1 - SEARCH BASELINE KerasMLP (random search)")
    ...
    # ============================================================
    # STAGE 2 - Restricted hybrids (random search)
    # ============================================================
    ...
    try:
        ...
        logger.error(f"Final comparison failed: {e}", exc_info=True)

    try:
        # ============================================================
        # CONSOLIDATION (like consolidar_resultados.py)
        # ============================================================
        logger.info("=" * 72)
        logger.info("CONSOLIDATION - ranking, tables, plots")
        consol_dir = os.path.join(OUT_DIR, "consolidated_analysis")
        consolidate_results(
            base_dir=Path(OUT_DIR),
            out_dir=Path(consol_dir),
            dataset_stem=dataset_stem,
        )
        logger.info(f"All done. Results in: {OUT_DIR}")
    except Exception as e:
        logger.error(f"Consolidation failed: {e}", exc_info=True)
        logger.info(f"Partial results in: {OUT_DIR}")
